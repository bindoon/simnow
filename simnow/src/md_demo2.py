...
APP_ID = get_env("APP_ID", "simnow_client_test")
AUTH_CODE = get_env("AUTH_CODE", "0000000000000000")
...
class MyMdSpi(ctp.mdapi.CThostFtdcMdSpi):
    def __init__(self, api, instrument_ids):
        super().__init__()
        self.api = api
        self.instrument_ids = [str(instrument_id).encode("ascii") for instrument_id in instrument_ids]

    def OnFrontConnected(self):
        print(f"连接成功，开始登录到第二套环境: {MARKET_FRONT}")
...
def main():
    ...
    api.RegisterSpi(spi)
    api.RegisterFront(MARKET_FRONT)

    print("使用 SimNow 第二套环境")
    print(f"BrokerID: {BROKER_ID}")
    print(f"Trade Front: {TRADE_FRONT}")
    print(f"Market Front: {MARKET_FRONT}")
    print(f"AppID: {APP_ID}")
    print(f"AuthCode: {AUTH_CODE}")
    print("注意: 当前 openctp_ctp.mdapi 绑定未暴露显式 ReqAuthenticate 接口，示例仅发起行情登录请求。")
